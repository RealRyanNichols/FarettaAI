...
const faqs = [
  ...
  },
  {
    ...
    a: "Cost. Opus 4.7 is excellent and expensive; Haiku 4.5 is fast and cheap. The gradient matches the stakes — an operator running a dental practice on KPIs wants the deep model; a mom asking where to log groceries is fine on Haiku. Everyone gets the same Faretta. Only the thinking horsepower changes.",
  },
  {
    q: "Can I cancel?",
...
export default function PricingPage() {
  return (
    <>
      ...
      <header className="relative pt-[140px] pb-20 px-6 max-w-f-desktop mx-auto">
        <motion.div
        ...
        >
          ...
          <h1 className="font-display font-display-weight leading-[0.98] tracking-[-0.03em] max-w-[900px] mb-6 text-[clamp(2.25rem,7vw,4.5rem)]">
            Pick how you meet
            <br />
            <span className="bg-f-wordmark bg-clip-text text-transparent">Faretta.</span>
          </h1>

          <p className="text-mist max-w-[640px] mb-4 leading-[1.5] text-[clamp(1.0625rem,2vw,1.25rem)]">
            Three tiers. Same Faretta in all of them — same voice, same evidence rule. What
      ...
      <section className="pb-[100px] px-6 max-w-f-desktop mx-auto">
        <div className="grid grid-cols-3 gap-5 max-[880px]:grid-cols-1">
          {tiers.map((t) => (
            <motion.div
              ...
              transition={{ duration: 0.4 }}
              className={[
                "relative rounded-f-primary p-7 border",
                t.emphasized
      ...
      <section className="py-[80px] px-6 max-w-f-desktop mx-auto border-t border-ink/[0.08]">
        ...
        <h2 className="font-display font-display-weight leading-[1.05] tracking-[-0.02em] max-w-[720px] mb-10 text-[clamp(1.75rem,4vw,2.75rem)]">
          One subscription. Five products. Same Faretta.
        </h2>
        <div className="grid grid-cols-2 gap-4 max-[720px]:grid-cols-1">
          {products.map((p) => (
            <div
              key={p.slug}
              className="rounded-f-secondary border border-ink/10 p-5 bg-[rgba(18,22,35,0.3)]"
            >
      ...
      <section className="py-[80px] px-6 max-w-f-desktop mx-auto border-t border-ink/[0.08]">
        ...
        <p className="text-mist text-[1.0625rem] max-w-[640px] leading-[1.6] mb-4">
          The first Faretta reduced his force from thirty-two thousand to three hundred,
        ...
        <p className="text-mist text-[1.0625rem] max-w-[640px] leading-[1.6]">
          This Faretta runs on the same principles: test the signal (every answer cites
          the network), keep the force small (short, direct, no hedging), and light the
          jar at exactly the moment it matters.
        </p>
      </section>

      <section id="faq" className="py-[80px] px-6 max-w-f-desktop mx-auto border-t border-ink/[0.08]">
        <span className="inline-block text-[0.75rem] tracking-[0.18em] uppercase text-gold font-semibold mb-4">
          Straight answers
        </span>
        <h2 className="font-display font-display-weight leading-[1.05] tracking-[-0.02em] max-w-[720px] mb-10 text-[clamp(1.75rem,4vw,2.75rem)]">
        ...
        <div className="space-y-4 max-w-[760px]">
          {faqs.map((f) => (
            <details
              key={f.q}
              className="rounded-f-secondary border border-ink/10 bg-[rgba(18,22,35,0.3)] p-5 open:border-gold/40 transition-colors"
            >
      ...
      <section
        id="get-started"
        className="py-[100px] px-6 max-w-f-desktop mx-auto text-center border-t border-ink/[0.08]"
      >
