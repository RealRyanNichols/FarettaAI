...
Faretta is the AI product Ryan Nichols is building across his entire
product portfolio. This folder is Faretta's constitution. Every repo
Ryan owns should reference it. Every Claude Code session that touches
a Faretta-integrated feature should read `CLAUDE.md` first.

---

## What Faretta is, in one paragraph

Faretta is an AI companion that wraps Claude but has its **own prompts,
its own memory layer, and its own personality**. Faretta collects wisdom
...
to any member whose current question is relevant. Faretta is the reason
...
*"The sword of the LORD, and of Faretta."* Every theme Ryan asked for
...
When Ryan integrates Faretta into another of his products:

1. Clone this folder into the new repo as `Faretta-AI/` (submodule or copy).
2. Add `Faretta-AI/CLAUDE.md` to the top of that repo's CLAUDE.md so
   every agent session reads the canonical source.
3. Add a product overlay file at `Faretta-AI/PROMPTS/product-overlays/<product>.md`
...
5. Faretta now works in that product with the same personality, the
