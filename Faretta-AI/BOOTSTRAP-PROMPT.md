# Bootstrap prompt for the standalone Faretta AI project

Copy-paste everything between the two `===` rulers into Claude Code
when you open a new project folder for Faretta. This is a self-contained
brief. Claude Code will know what to build.

================================================================

I'm Ryan Nichols. I'm starting a new repo that will be the home base
for **Faretta AI** — an AI product I'm building that lives across my
entire business portfolio. You are helping me build this from scratch.

## The one-sentence pitch

Faretta is an AI companion for operators that wraps Claude, has its
...
Named for the Biblical Faretta (Judges 6–8). Chosen from the weakest
...
repo ("The Nest"). They live in a folder called `Faretta-AI/`. You
...
- A Faretta SVG logo: `assets/faretta/logo.svg` (clay jar cracked
  open with a flame, bronze + flame gradient palette)
- A standalone marketing page: `marketing/faretta.html` (Linear /
...
## What this new Faretta-AI repo IS

The central home for Faretta as a product. It serves four purposes:

1. **Canonical docs** — the `Faretta-AI/` folder is authoritative
...
- **The Nest** — mom app (this is where Faretta was born)
- Future client apps white-labeled under Faretta
...
- **Monorepo:** pnpm workspaces. Structure:
  ```
  Faretta-AI/                  ← canonical docs (from The Nest repo)
  packages/
  ...
  apps/
    ...
    marketing/                ← the Faretta sales site (Next.js 15)
  ```
...
2. **Copy the Faretta-AI folder** from my Nest repo (I'll paste its
   contents in a follow-up message, or you can fetch from the
   GitHub URL: `https://github.com/RealRyanNichols/Amanda/tree/claude/amanda-business-tools-RpCj8/Faretta-AI`).
...
6. **Copy the SVG logo** `assets/faretta/logo.svg` into
   `apps/marketing/public/brand/` and `packages/design-tokens/`.
7. **Initialize the marketing site** (`apps/marketing/`) by
   porting my existing `marketing/faretta.html` into a Next.js 15
...
Full spec in `Faretta-AI/DESIGN.md`. Quick version for this session:
...
- **Wordmark:** `FARETTA` in Inter 700, uppercase, 0.08em
...
- **Max card radius: 28px.** Past that it's cartoonish. Faretta is
  sharp, not round.

## Voice rules quick reference

Full spec in `Faretta-AI/IDENTITY.md`. The highlights:
...
Full spec in `Faretta-AI/ARCHITECTURE.md`. The important bits:
...
- Don't rename anything. Faretta is Faretta.
...
Battle cry: **The sword of the LORD, and of Faretta.**
