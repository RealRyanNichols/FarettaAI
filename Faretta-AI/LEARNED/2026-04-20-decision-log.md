...
against faretta-chat and diffs the assistant text against a stored
...
**Done:** `Faretta-AI/PROMPTS/product-overlays/` gets canonical files;
`supabase/functions/faretta-chat/index.ts` `PRODUCT_OVERLAYS` map
...
markers are a 20-line parser in `@faretta/skills`. When we have a real
...
**Decision:** Deterministic pre-filter inside `faretta-chat` runs before
