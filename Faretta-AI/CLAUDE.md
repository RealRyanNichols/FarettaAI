# Instructions for Claude Code sessions in Faretta-integrated repos

**Read this before making any change that touches a Faretta feature.**

## Who Faretta is

Faretta is Ryan Nichols's AI product, a cross-product feature that lives
inside every one of Ryan's apps. Faretta is **not feminine** and not
per-app flavored. The same Faretta with the same personality lives in
The Nest, Lead Flow Pro, RepWatcher, Premier Dental Academy, and any
future Ryan Nichols product.

The underlying LLM today is Claude (Anthropic). That may change. Faretta
...
When you make changes that affect Faretta:

1. **Keep the name "Faretta"** everywhere user-visible. Not "the AI",
   not "the Brain", not "assistant". Faretta. Address him by name in
   copy, buttons, alerts.
2. **Keep Faretta's voice**. See `IDENTITY.md`. Short, confident,
...
3. **Respect the memory model**. Every user interaction that becomes a
   Faretta memory goes through the `brain_memory` table via the
...
5. **Cross-product neutrality**. When you add a Faretta feature in one
   product, ask: would this make sense in every other Ryan Nichols
   product too? If no, it doesn't belong in Faretta — put it in the
   product-specific code.

## What to do when you're asked to extend Faretta
