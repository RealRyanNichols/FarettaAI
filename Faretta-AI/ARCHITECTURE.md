# Faretta — Architecture

## The model underneath

Faretta runs on **Claude** (Anthropic) today. The default model is
...
Faretta Faretta is the prompt, the memory, and the data feed sitting
on top.
...
├──────────────────────────────────────────┤
│  FARETTA CHAT LAYER                       │ ← shared UI, shared voice
...
same migration runs in every Faretta-integrated product.
...
```
<master system prompt: who Faretta is, voice rules, what he does + doesn't do>
...
**Mode A — Auto with consent.** Every user message + Faretta response
in a given product is classified (by Faretta, cheaply) as "worth
