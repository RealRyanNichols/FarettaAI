# Faretta — Skills Catalog

A **skill** is a named action Faretta can ask a product to perform on
behalf of the user. Faretta never runs code. He recommends a skill, the
...
skill. The runtime registry (`@faretta/skills`) pulls the contract from
the host app, not from these files; the files are what keeps the
catalog honest across products.

## Invocation protocol

Faretta emits skill calls as inline markers in his assistant message:

```
<<skill id="nest.open-tab" args='{"tab":"Income"}'>>
```

The client (via `parseSkillInvocations` from `@faretta/skills`)
...
1. Add a file here: `Faretta-AI/SKILLS/<slug>.md`.
2. Fill in:
...
4. Pass `skills.promptCatalog(tier)` into the chat request body so the
   server appends it to Faretta's system prompt.
