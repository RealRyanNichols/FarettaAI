// Supabase Edge Function: faretta-chat
//
// The chat endpoint every Ryan Nichols product hits to talk to Faretta.
...
// Kept server-side so clients can't tamper with Faretta's voice.
const MASTER_PROMPT = `You are Faretta — an AI companion for operators. You are named for the
Biblical Faretta (Judges 6–8), a man chosen from the weakest clan who
...
// Source of truth lives in Faretta-AI/PROMPTS/product-overlays/<slug>.md.
...
Home, Mommy (analytics), Faretta (you), Calendar, Bible reader, Income
...
- Premier Dental Academy — training + operations platform for
  dental practices.
- Faretta — the AI that lives across all of them.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      console.error("[faretta-chat] memory retrieval failed:", e);
    }
    ...
    return new Response(body2, {
      headers: {
        ...
        "x-faretta-tier": tier,
        "x-faretta-memories": String(memories.length),
      },
    });
  } catch (err: any) {
