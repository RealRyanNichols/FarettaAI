...
import type { ChatMessage, GideonChatProps } from "./types";
import { useGideonStream } from "./useGideonStream";
...
// @gideon/design-tokens; otherwise the inline fallbacks apply.
//
...
//     return (
//       <GideonChat
//         endpoint="https://<ref>.functions.supabase.co/gideon-chat"
//         project="nest"
//         tier="core"
//         userName="Amanda"
...
export function GideonChat(props: GideonChatProps) {
  const {
    endpoint, project, tier, userName, userContext,
    initialHistory = [], defaultOpen = false, triggerLabel = "Ask Gideon",
    onTurnComplete,
  } = props;
  ...
  const { streaming, draft, error, send, cancel } = useGideonStream(streamOpts);
  ...
  const submit = useCallback(async () => {
    ...
    try {
      ...
      // error already surfaced via useGideonStream's error state.
    }
  ...
  return (
    ...
    >
      ...
      >
        <span style={{ fontWeight: 700, fontSize: 15, letterSpacing: 0.3 }}>Gideon</span>
        <button
          type="button"
          onClick={() => setOpen(false)}
          aria-label="Close Gideon"
          style={{
