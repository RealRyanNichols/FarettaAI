  ...
  while (true) {
    ...
    for (const ev of events) {
      ...
      try {
        ...
        console.warn("[gideon-ui] malformed SSE frame:", data);
      }
    }
  }
}

// React hook that wraps a single POST to the gideon-chat endpoint
// and streams the assistant's text back one delta at a time.
export function useGideonStream(opts: Pick<GideonChatProps,
  ...
  const send = useCallback(async ({ message, history }: StreamArgs): Promise<string> => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(`gideon-chat ${res.status}: ${errBody.slice(0, 200)}`);
      }
