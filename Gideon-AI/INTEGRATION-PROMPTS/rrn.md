...
`RealRyanNicholsLLC` repository. The session will scaffold Gideon
...
1. The GideonAI Supabase project is deployed with the four Edge
   Functions (`brain-ingest`, `brain-query`, `gideon-chat`,
   `brain-summarize`). Its project-ref is recorded below.
2. A Vercel project named `realryannichols-com` (or equivalent) is
   already connected.
3. The GideonAI repo is reachable either as a workspace package or
   as a git submodule at `vendor/gideon/`.

Fill the three values in the prompt before pasting:

- `<GIDEON_SUPABASE_REF>` — the Supabase project-ref for GideonAI.
- `<GIDEON_CHAT_URL>` — full URL like
  `https://<ref>.functions.supabase.co/gideon-chat`.
- `<GIDEON_PACKAGES_PATH>` — how this repo consumes `@gideon/*`. One
...
```
# Claude Code — integrate Gideon AI into www.realryannichols.com

You are operating inside the RealRyanNicholsLLC repository, which
hosts www.realryannichols.com. Your job is to integrate the Gideon
AI product (from the `realryannichols/gideonai` repo) into this site
as a first-class feature. Ryan's email must NEVER appear on any
user-visible surface — all visitor contact flows through the Gideon
contact form, which writes to Supabase.

## Canonical context

- Gideon hub repo: realryannichols/gideonai
- Source of truth for behavior: Gideon-AI/ directory inside that repo.
...
- Shared packages you will install: @gideon/ui, @gideon/skills,
  @gideon/brain-memory, @gideon/design-tokens.
- Deployed Edge Function: <GIDEON_CHAT_URL>
- GideonAI Supabase project-ref: <GIDEON_SUPABASE_REF>
- Package source: <GIDEON_PACKAGES_PATH>

## Non-negotiables

1. This site's project slug is `rrn`. Every call to
   `setProject()` / `<GideonChat project="...">` / the chat API uses
   exactly `"rrn"`.
2. The rrn product overlay (Gideon-AI/PROMPTS/product-overlays/rrn.md)
   is the canonical voice. Do not fork it — if the overlay needs to
   change, open a PR in realryannichols/gideonai under
...
6. Keep the visual change minimal. The rrn site has its own brand;
   Gideon is a guest inside it, not a takeover. Use @gideon/design-
   tokens variables through a CSS var prefix scoped to the chat
   widget — don't push Gideon colors into site-level styles.
...
Pick the right install path based on <GIDEON_PACKAGES_PATH>:

- Workspace: add `"@gideon/ui": "workspace:*"`, etc. to
  `package.json`, then `pnpm install` (or `npm install`).
- Submodule: `git submodule add git@github.com:realryannichols/gideonai.git vendor/gideon`,
  then add `"@gideon/ui": "file:./vendor/gideon/packages/ui"` etc.
  Build the packages once:
  `pnpm --filter @gideon/brain-memory --filter @gideon/skills --filter @gideon/ui build`.
- Registry: once Ryan publishes, `pnpm add @gideon/ui @gideon/skills @gideon/brain-memory @gideon/design-tokens`.
...
```
NEXT_PUBLIC_GIDEON_CHAT_URL=<GIDEON_CHAT_URL>
NEXT_PUBLIC_SUPABASE_URL=https://<GIDEON_SUPABASE_REF>.supabase.co
...
Create `lib/gideon.ts` (or equivalent):
...
export function getGideonAdminClient() {
...
Create a client component `components/GideonMount.tsx`:

```tsx
"use client";
import { GideonChat } from "@gideon/ui";

export function GideonMount() {
  return (
    <GideonChat
...
const GideonMount = dynamic(() => import("@/components/GideonMount").then(m => m.GideonMount), { ssr: false });
```

## Phase 6 — register rrn-specific skills

Create `lib/gideon-skills.ts`:
...
// Route the visitor to a product page when Gideon recommends one.
rrnSkills.register({
...
// Route the visitor to the contact form (Gideon never surfaces an
...
`components/GideonMount.tsx`:

```tsx
"use client";
import { GideonChat } from "@gideon/ui";
import { useMemo } from "react";
import { rrnSkills } from "@/lib/gideon-skills";
...
(Note: the current @gideon/ui version does not yet parse skill
...
- The form POSTs to `/api/contact` server-side and writes to the
  GideonAI Supabase `contact_requests` table via service-role.
  See `apps/marketing/app/api/contact/route.ts` in the gideonai repo
...
3. `pnpm dev`, open http://localhost:3000, click the floating
   "Ask Gideon" button. Send "hey". You should see:
   - A POST to `<GIDEON_CHAT_URL>` with status 200.
   - Response header `x-gideon-model` set.
   - Response header `x-gideon-memories` set (likely 0 on a fresh
     brain).
...
5. Ask Gideon a safety-triggering question ("I want to hurt myself")
   and confirm the response is the 988 canned line with header
   `x-gideon-crisis: suicide`. The model should NOT be invoked.
...
Push to a branch named `gideon/integrate-rrn` and open a DRAFT PR
against `main`. The PR body should link to:

- Gideon-AI/INTEGRATION.md in the hub repo.
- Gideon-AI/PROMPTS/product-overlays/rrn.md (the voice Gideon uses here).
...
- Any ambiguity about whether a change affects Gideon's cross-product
...
- Tell the GideonAI repo about any rrn overlay corrections via a
  dated LEARNED note.
